...
func dialer() func(context.Context, string) (net.Conn, error) {
	...
	conn, err := grpc.Dial(fmt.Sprintf("crypto_service:%d", cfg.CryptoServicePort), grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		mLogger.Error(ctx, "failed crypto conn", zap.String("Error", err.Error()))
		log.Fatal(err)
	}
	client := crypto.NewCryptoServiceClient(conn)
	file.RegisterFilesServiceServer(server, NewFileService(srv, mLogger, client))
...
func TestUploadFile(t *testing.T) {
	...
	md := metadata.New(map[string]string{"X-User-ID": "tset user id"})
